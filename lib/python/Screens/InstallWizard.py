...
# 	STATE_CHOISE_SOFTCAM = 2
	...
	def __init__(self, session, args = None):
		...
		if self.index == self.STATE_UPDATE:
			...
			modes = {"19e": "Astra 19e", "19e-13e": "Astra 19e + Hotbird 13e"}
...
# 				self.list.append(getConfigListEntry(_("Softcam type"), self.softcam_type))
	...
	def run(self):
		if self.index == self.STATE_UPDATE:
			...
			self.session.open(InstallWizardIpkgUpdater, self.index, _('Please wait (downloading channel list)'), IpkgComponent.CMD_REMOVE, {'package': 'enigma2-plugin-settings-matze-' + self.channellist_type.value})
