...
config.pluginfilter.atemio = ConfigYesNo(default = False)
...
def Check_Softcam():
	found = False
	if fileExists('/usr/lib/enigma2/python/Plugins/PLi/SoftcamSetup/plugin.pyo'):
		found = True
	return found
...
class PluginDownloadBrowser(Screen):
	...
	def createPluginFilter(self):
		...
		if config.pluginfilter.atemio.value:
			self.PLUGIN_PREFIX2.append(self.PLUGIN_PREFIX + 'atemio')
	...
	def dataAvail(self, str):
		...
		for x in lines:
			...
			if len(plugin) >= 1:
				if not plugin[0].endswith('-dev') and not plugin[0].endswith('-staticdev') and not plugin[0].endswith('-dbg') and not plugin[0].endswith('-doc') and not plugin[0].endswith('-src') and not plugin[0].endswith('-po') and not plugin[0].endswith('-meta'):
					# Plugin filter
...
class PluginFilter(ConfigListScreen, Screen):
	...
	def createSetup(self):
		...
		self.list.append(getConfigListEntry(_("atemio"), config.pluginfilter.atemio, _("This allows you to show atemio modules in downloads")))
