...
#			self.secondInfoBarScreen.hide()
	...
	def keyHide(self):
		if self.__state == self.STATE_HIDDEN:
			if config.atemiosettings.InfoBarEpg_mode.value == "2":
				self.openInfoBarEPG()
			else:
				self.hide()
...
class InfoBarChannelSelection:
	...
	def LeftPressed(self):
		if config.atemiosettings.InfoBarEpg_mode.value == "3" and config.usage.show_second_infobar.value != "INFOBAREPG":
			self.openInfoBarEPG()
		else:
			self.zapUp()
...
class InfoBarExtensions:
	...
	def getCCcamInfo(self):
		if pathExists('/etc/init.d/'):
			softcams = os.listdir('/etc/init.d/')
		for softcam in softcams:
			if softcam.lower().endswith('cccam') and config.cccaminfo.showInExtensions.value:
	...
	def getOScamInfo(self):
		if pathExists('/etc/init.d/'):
			softcams = os.listdir('/etc/init.d/')
		for softcam in softcams:
			if softcam.lower().endswith('oscam') and config.oscaminfo.showInExtensions.value:
