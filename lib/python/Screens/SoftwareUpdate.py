...
class UpdatePlugin(Screen):
	...
	def ipkgCallback(self, event, param):
		if event == IpkgComponent.EVENT_DOWNLOAD:
			...
			if self.updating:
				...
				config.softwareupdate.updateisunstable.setValue(urlopen("http://feed.atemio4you.com/" + getImageVersion() + "/status").read())
				...
				if self.total_packages:
					...
					if path.exists("/usr/lib/enigma2/python/Plugins/Extensions/AtemioPanel/BackupManager.pyo"):
	...
	def doSettingsBackup(self):
		backup = None
		from Plugins.Extensions.AtemioPanel.BackupManager import BackupFiles
		self.BackupFiles = BackupFiles(self.session, True)
		Components.Task.job_manager.AddJob(self.BackupFiles.createBackupJob())
		Components.Task.job_manager.in_background = False
		for job in Components.Task.job_manager.getPendingJobs():
			if job.name == dgettext('atemio', 'Backup Manager'):
				break
		self.showJobView(job)

	def doImageBackup(self):
		backup = None
		from Plugins.Extensions.AtemioPanel.ImageManager import ImageBackup
		self.ImageBackup = ImageBackup(self.session, True)
		Components.Task.job_manager.AddJob(self.ImageBackup.createBackupJob())
		Components.Task.job_manager.in_background = False
		for job in Components.Task.job_manager.getPendingJobs():
			if job.name == dgettext('atemio', 'Image Manager'):
				break
		self.showJobView(job)
	...
	def showJobView(self, job):
		...
		elif job.name == dgettext('atemio', 'Backup Manager'):
