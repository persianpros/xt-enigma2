...
class PliExtraInfo(Poll, Converter, object):
	...
	def getText(self):
		...
		if self.type == "ServiceInfoFreq":	
			return addspace(self.createFrequency(feraw)) + ' POL : ' + addspace(self.createPolarization(fedata)) + ' FEC : ' + addspace(self.createFEC(fedata, feraw)) \
			+ ' SR : ' + addspace(self.createSymbolRate(fedata, feraw)) + ' MOD : ' + addspace(self.createModulation(fedata))
